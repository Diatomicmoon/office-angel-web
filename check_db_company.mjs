...
async function check() {
  const { data, error } = await supabase.from('companies').select('id, name, phone_number, ai_enabled, forward_to_phone').limit(1);
  console.log("Current company state:", JSON.stringify(data[0], null, 2));
}
check();

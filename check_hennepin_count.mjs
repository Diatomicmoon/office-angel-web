...
async function run() {
   const { count, error } = await supabase.from('hennepin_parcels').select('*', { count: 'exact', head: true });
   console.log("Total properties loaded into hennepin_parcels table:", count);
}
run();

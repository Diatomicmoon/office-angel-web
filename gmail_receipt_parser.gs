// Office Angel - Gmail Ingestion Script
// Paste this into script.google.com

const SUPABASE_URL = "https://ztknhbilfergfwoxjzvb.supabase.co";
const SUPABASE_KEY = "YOUR_SUPABASE_ANON_KEY"; // We will put the real one here
const OPENAI_API_KEY = "YOUR_OPENAI_API_KEY"; // You'll need to generate one at platform.openai.com

// 1. Main function that runs every 5 minutes
function processOfficeAngelEmails() {
  // Find unread emails with receipts or permits
  ...
  for (let i = 0; i < threads.length; i++) {
    ...
    for (let j = 0; j < messages.length; j++) {
      ...
      if (msg.isUnread()) {
        ...
        // Step 2: Send to OpenAI to extract data
...
// 2. Call OpenAI to parse the messy email text into clean JSON
...
// 3. Save the clean JSON directly to your Supabase database
function saveToSupabase(data, sender) {
  const payload = {
    "supplier_name": data.supplier_name || sender,
    "total_amount": data.total_amount || 0,
    "line_items": data.line_items || [],
    "status": "Action Required" // Drops it into the dashboard for review
  };
